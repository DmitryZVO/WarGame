    ...
    protected sealed override void DrawUser()
    {
        lock (this)
        {
            ...
            if (obj != null) // Объект выбрат
            {
                ...
                Rt?.DrawText($"OBJ_IN: {obj.Telem.MBitObjectIn:0.000000} Mbit", Brushes.SysText14, new RawRectangleF(joyRect.Left + xLine * 0.06f, BaseHeight * 0.88f, BaseWidth, BaseHeight), Brushes.SysTextBrushRed);
                Rt?.DrawText($"SERV_IN: {obj.Telem.MBitServerIn:0.000000} Mbit", Brushes.SysText14, new RawRectangleF(joyRect.Left + xLine * 0.01f, BaseHeight * 0.90f, BaseWidth, BaseHeight), Brushes.SysTextBrushYellow);
                Rt?.DrawText($"PING<>UDP: {obj.Telem.PingToServer:0.00} ms", Brushes.SysText14, new RawRectangleF(joyRect.Left + xLine * 0.06f, BaseHeight * 0.92f, BaseWidth, BaseHeight), Brushes.SysTextBrushWhite);
                Rt?.DrawText($"COMMANDS: {obj.Telem.CommandCount:0}", Brushes.SysText14, new RawRectangleF(joyRect.Left + xLine * 0.14f, BaseHeight * 0.94f, BaseWidth, BaseHeight), Brushes.SysTextBrushOrange);
            }
